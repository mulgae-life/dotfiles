...
1. 사용자의 현재 요청 (신규 지시가 이전 지시와 충돌하면 신규 우선; 충돌 없는 이전 지시는 유지 — 5.5 §6.3)
2. 저장소의 실제 코드/구조/테스트 결과
3. 이 문서의 지침

> 응답 형식·범위 규율·코딩 스타일·언어 규칙·작업 사고(outcome-first §3)·협업 스타일(§4)·도구 사용(§5)·검증(§7.1)은 `~/.codex/config.toml`의 `developer_instructions`에 정의되어 있다. 이 문서는 **에이전트 운영·스킬 카탈로그·레퍼런스 검증·비파괴 원칙**에 집중한다.
...
`config.toml`의 `[features] multi_agent = true` 활성. `[agents]` `max_threads = 6`.
...
- **Starlark `forbidden` 명령은 시도 자체 금지**: 아래 명령은 `.codex/rules/default.rules`에서 `forbidden`으로 설정되어 **사용자 명시 요청 여부와 무관하게 차단**됩니다 (Codex Starlark는 ask 메커니즘이 없음). 자율 작업 중에는 절대 시도하지 말고, 사용자가 요청한 경우에는 **사용자에게 직접 실행을 부탁드린다고 안내**:
  ...
  - **Git 쓰기**: `git push`, `git commit`, `git reset`, `git clean`, `git rebase`, `git merge`, `git cherry-pick`, `git revert`, `git am`, `git apply`, `git rm`
  - **Git 상태 변경**: `git checkout`, `git switch`, `git restore`, `git stash` (전체), `git add`, `git branch -d/-D`, `git tag -d/-f` — 작업 컨텍스트/working tree/staging 상태 변경 위험
  ...
  - **파일 수정/링크 강제/권한**: `ln` (전체 차단, -f/-sf 위험), `chmod`, `chown`, `sed -i` (옵션 첫 인자 케이스만 매칭) — Edit 도구 우회·보안 상태 변경
...
> - `sed -i` 옵션이 뒤에 오는 케이스 (예: `sed -e 'pattern' -i file`)
> - `awk -i inplace` (gawk 인플레이스)
