...
| 구분 | 뜻 | 갱신 계기 |
|------|-----|-----------|
| **이식** | 공식 저장소의 스킬을 가져와 번역·조정한 것. 일부 기준만 차용한 `code-review`도 상류 대조가 필요해 여기 둔다 | 상류 저장소가 바뀌었을 때 (2절) |
...
| `office-docs` | `anthropics/skills` · `skills/pdf/`·`skills/docx/`·`skills/pptx/`·`skills/xlsx/` (GitHub이 claude.ai 배포본보다 뒤처질 수 있다 — 계정 동기화본이 더 새로우면 그쪽을 기준으로 삼는다) | 진입 `SKILL.md` 자체 작성(포맷 라우팅·실행 규약·공통 규칙). 네 상류 `SKILL.md` 본문을 프론트매터만 떼어 `references/<포맷>.md`로, pdf의 `forms.md`·`reference.md`는 `pdf-forms.md`·`pdf-reference.md`로 영문 원문 유지. 세 스킬에 중복된 `scripts/office/`는 한 벌만 두고 나머지 스크립트는 `scripts/` 평면에 모음(이름 충돌 없음, `from office…` import가 `scripts/`를 sys.path로 가정하기 때문) | 참조 본문: "preinstalled" 전제 문구를 `setup.sh --check`·venv·`NODE_PATH` 안내로, `unzip`+`find -delete`·`rm -f`+`zip -Xr`·`rm -f slide-*.jpg`를 `unpack.py`·`pack.py`·렌더별 새 디렉토리로, pdf의 `FORMS.md`·`REFERENCE.md` 파일명을 새 이름으로 바꾼 것. `pdf-forms.md`는 참조가 `references/` 아래로 내려가 상류의 "from this file's directory"가 더는 맞지 않아 스킬 루트 기준으로 고쳤고, 상류가 `.py`를 빠뜨린 `check_fillable_fields` 표기도 함께 바로잡았다. 자체 추가 파일 `scripts/setup.sh`·`scripts/pack.py`·`scripts/unpack.py`는 상류에 없다 |
