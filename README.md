...
**명령어 자동 승인:** `hooks/`의 PreToolUse 훅이 Bash 명령어를 실행 전에 검사한다. 상태를 변경하지 않는 읽기 전용 명령어(`ls`, `git status`, `cat`, 파이프 조합 등)는 자동 승인되고, 상태를 변경하는 명령어(`rm`, `git push`, `sudo` 등)는 사용자 확인을 요청한다.
