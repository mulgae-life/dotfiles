...
| Linux | IDE 미지원 → `~/.antigravity`, `~/.gemini/antigravity-cli/skills` (agy CLI만) | symlink |
...
`.claude/hooks/auto-approve-readonly.sh`를 그대로 재사용하므로 11 카테고리 분류와 사유 메시지가 4-tool 전체에서 정확히 일치합니다. (PROCESS는 v2.2에서 allow로 해제 — 재시작 가능한 조작)
