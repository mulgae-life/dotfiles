...
`GEMINI.md` §작업원칙 §금지명령 참조. 12 카테고리(FILE_DELETE / SYSTEM / GIT_WRITE / GIT_STATE / GH_CLI / DOCKER_DELETE / INPLACE / LINK_FORCE / PERMISSION / PROCESS / SHELL_BYPASS / SCRIPT_INJECTION) 모두 Antigravity의 `permissions.deny` + `before_tool_call` 훅으로 강제됩니다 (`.antigravity/settings.json`).
