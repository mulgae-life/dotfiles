---
name: skill-creator
description: 효과적인 스킬 생성 가이드. 새 스킬 생성 또는 기존 스킬 업데이트 시 사용. Claude의 기능을 확장하는 전문 지식, 워크플로우, 도구 통합을 정의합니다. "스킬 만들어줘", "스킬 수정해줘" 등의 요청에 트리거.
---
...
| **Opus** (powerful reasoning) | Does the Skill avoid over-explaining? Can handle more ambiguity. |
...
Clear steps prevent Claude from skipping critical validation. The checklist helps both Claude and you track progress.

##### Feedback Loop Pattern

Implement "run → validate → fix → repeat" loops to improve output quality:
...
3. If errors found:
   ...
   - Return to step 1
4. If no errors: proceed to next phase

**Maximum iterations**: 3 (ask user if still failing)
````
