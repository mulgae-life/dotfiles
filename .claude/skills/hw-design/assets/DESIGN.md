...
브랜드 에센스 "Energy for Life" + 트리서클(조화·확장·신뢰)을 디지털 표면으로 번역.

- **톤 키워드**: 격식 · 신뢰 · 절제 · 명료 · 안정
- **정보 밀도**: 중간~높음. 격식 있는 비즈니스 UI 톤 (SaaS 대시보드보다 여유, 미디어형보다 빠듯).
- **첫 인상**: "오랜 그룹의 안정감" — 화려함보다 절제, 트렌드보다 계승.
- **회피 인상**: 캐주얼 · 플레이풀 · 네온 · 실험적.
...
- **줄바꿈 (기본)**: `word-break: keep-all; overflow-wrap: break-word; line-break: strict;` — 단어 단위 끊김 방지 + 엄격 금칙 처리.
- **금칙 (행두 금지)**: 닫는 괄호·구두점이 줄 머리에 오지 않도록. `line-break: strict`이 자동 처리. 대상: `)」』】〕》、。，．・：；？！`.
- **금칙 (행말 금지)**: 여는 괄호가 줄 끝에 오지 않도록. 대상: `(「『【〔《`.
- **숫자·단위 묶기**: `100원`, `2026년` 등 숫자와 단위 사이 줄바꿈 금지 — `<span class="nobr">` 또는 `&nbsp;` 사용.
...
타이포 정밀도 룰. `font-feature-settings` 또는 `font-variant-*`로 적용.

| 영역 | 권장 feature | 이유 |
|------|--------------|------|
| 한글 본문 (`body`, `body-lg`, `body-sm`) | (기본, feature 없음) | `palt` 적용 시 한화고딕의 의도된 자간이 깨진다 — 본문엔 적용 금지 |
| 한글 헤딩 (`display`, `h1`, `h2`) | `'palt' 1, 'kern' 1` | 한화체 큰 사이즈에서 자간 정밀화 (특히 괄호·구두점 주변) |
| 영문·숫자 (`numeric`, `label-caps`, `button`) | `'tnum' 1, 'lnum' 1, 'kern' 1, 'liga' 1` | 표·대시보드 자릿수 정렬 + 영문 합자 + 라이닝 숫자 |
| 모노 코드 (해당 시) | `'tnum' 1, 'liga' 0` | 정렬 우선, ligature 비활성 |

```css
/* 예: 헤딩 */
h1, h2, .display { font-feature-settings: 'palt' 1, 'kern' 1; }

/* 예: 숫자 지표 */
.numeric { font-feature-settings: 'tnum' 1, 'lnum' 1, 'kern' 1, 'liga' 1; }
```
...
여백은 한화 톤의 핵심이다 — **여백이 곧 권위**. 정보 밀도를 욕심내지 않는다.

- **섹션 사이 ≥ `xxl(48)`, 카드 사이 ≥ `md(16)`**: "비어 보임"의 두려움보다 격을 우선.
- **CTA 주변 클리어 존**: 좌우 ≥ `lg(24)`, 상하 ≥ `md(16)` — 강조는 면적이 아니라 여백이 만든다.
- **테이블·리스트 행간**: `body line-height × 1.4` 이상 — 빠듯하게 채우지 않는다.
- 화려한 일러스트나 가득 찬 그라디언트로 "있어 보이게" 만들지 않는다. 한화의 무게는 침묵이다.
