...
| `--hw-orange-tint` | `#FCE6D6` | `252, 230, 214` | 본문 wash, As-Is/To-Be 패널, 테이블 호버, **헤더 strip 배경** |
...
| `--hw-mist` | `#F5F5F5` | `245, 245, 245` | 표 alt 행 배경 (~~헤더 strip~~ — paper와 명도차 부족하여 orange-tint로 대체) |
...
- 본문: line-height **1.55–1.7** (필수 명시 — 미지정 시 single line이라 답답)
- 타이틀: line-height **1.1**
...
| 항목 | 룰 |
|------|------|
| 한글 타이틀 박스 height | **font_px × 1.6 이상** (descender + 두 줄 wrap 안전. ×1.4 이하는 PowerPoint 실측에서 부족) |
| 서브타이틀 y 위치 | **title 박스 끝 + 24 px** (font_px 기준 계산은 부정확 — title 실제 box height 추적) |
| 부제-본문 vertical gap | **최소 30 px** (0~10 px gap은 답답) |
| 멀티라인 본문 line_spacing | **1.55+** 명시 |
| 박스 간 vertical gap | **최소 12 px** (0 px은 시각상 답답) |
| 같은 라인 박스 anchor | **middle 통일** (번호+제목 등 폰트 크기 다를 때 baseline 어긋남 방지) |
...
| `--header-h` | `80 px` | 헤더 strip 높이 (시그니처 이미지 64 px 수용) |
| `--signature-h` | `64 px` | 헤더 시그니처 이미지 height (`hanwha-signature-ink.png`, 비율 2:1) |
| `--margin-x` | `80 px` | 슬라이드 좌측 마진 (기본) |
| `--margin-header` | `40 px` | 헤더 좌우 인셋 |
| `--title-band-top` | `130 px` | 타이틀 밴드 시작 (헤더 80 확장 기준) |
