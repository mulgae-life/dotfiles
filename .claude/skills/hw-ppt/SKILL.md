...
1. **9개 아키타입을 좌표 수준으로 고정** — 표지(y=110), 헤더(y=0–56), 타이틀밴드(y=110–270), 본문(y=300–820), Density Zone(y=840–1010)
2. **Density Zone 룰** — 슬라이드 하단의 빈 공간 문제 해결 (§7)
...
| **로고** | `hanwha-symbol.png`만 사용 — 옆에 "한화손보" wordmark 병기 |
| **헤더** | 모든 콘텐츠 슬라이드에 동일 좌표 (y=0–56, x 마진 40 px 좌우 동일) |
| **타이틀** | y=110–270 밴드 고정 (좌·중앙 정렬은 아키타입별로 다름, 밴드 위치는 불변) |
| **밀도** | Density Zone(y=840–1010) 비워두지 않음 (§7) |
...
**고정 좌표 (슬라이드 간 변동 금지):**
- 헤더 strip: 전체 너비, **y = 0–56**, 배경 `--hw-mist`, 1px `--hw-line` 하단선
- 헤더 챕터 라벨: 왼쪽 가장자리에서 **40 px 안쪽**, 56px strip 세로 중앙
- 헤더 시그니처(심볼+"한화손보"): 오른쪽 가장자리에서 **40 px 안쪽**, 세로 중앙. 40 px 갭은 슬라이드 가장자리에서 **wordmark 가장 오른쪽 픽셀까지**의 거리 — 왼쪽 40 px와 시각적으로 동일
- 타이틀 밴드: **y = 110–270**, x = 80 (좌측 정렬 기본)
- 본문 zone: **y = 300–820**
- Density Zone: **y = 840–1010**
- 페이지 인디케이터(옵션): 오른쪽 가장자리 40 px 안쪽, **y = 1040**
...
콘텐츠 슬라이드의 공식 마크는 **시그니처**: `hanwha-symbol.png` 이미지 + "한화손보" wordmark(한화체 Bold).

```
  [hanwha-symbol.png, 28 px tall]  ⟵ 8 px gap ⟶  한화손보   ← 한화체 Bold 18 px, --hw-ink
```
...
- 표지/섹션 디바이더: 시그니처 높이 44 px로 확대 가능 (wordmark 26 px)
- **금지**: 색 변형, 회전, 미러링, 왜곡, 영문 wordmark로 교체, AI 생성 placeholder
...
- x=80 → 508,000 EMU
- y=56 → 355,600 EMU
- 1 inch = 914,400 EMU, 표준 슬라이드 폭 = 12,191,996 EMU
...
- PPTX는 .ttf/.otf를 직접 임베드. 별도 변환 불필요.
- 한화고딕은 .woff2만 존재 — 본문에 한화고딕이 꼭 필요하면 fontTools로 .ttf 변환:
  ```bash
  pip install fonttools brotli
  python -c "from fontTools.ttLib import TTFont; f=TTFont('HanwhaGothicR.woff2'); f.flavor=None; f.save('HanwhaGothicR.ttf')"
  ```
...
- [ ] 헤더 strip (gray, 56 px, 챕터 라벨 좌·시그니처 우)가 모든 콘텐츠 슬라이드에 동일 좌표로 존재
- [ ] **헤더 좌우 마진 대칭** — 40 px 좌(챕터 라벨까지) / 40 px 우(wordmark 외곽까지). 눈으로 봐서 동일해야 함
- [ ] **시그니처는 실제 `hanwha-symbol.png` 파일을 `<img>`로 사용** — 생성 아이콘·이모지·simplified circle·AI placeholder 금지
- [ ] 심볼·wordmark는 `align-items: center`로 광학 중심 정렬 (manual offset 금지)
- [ ] 타이틀 밴드 y = 110–270, 슬라이드 간 동일
...
- **`assets/logo/`** — 시그니처용 로고 PNG (현재 JPEG 검정 배경 — alpha PNG 변환 필요 시 `references/pptx-implementation.md § 로고 알파 변환` 참조)
...
- 좌표(`y=110–270` 타이틀 밴드 등)를 모든 슬라이드에서 동일하게 유지
...
> **검정 배경 JPEG 로고 안내**: `assets/logo/hanwha-symbol.png`과 `hanwha-signature.png`는 현재 검정 배경 JPEG이다 (사용자가 클로드 웹에서 사용하던 원본). HTML deck은 검정 배경 헤더에서 문제 없으나, .pptx에선 알파 PNG 권장. 변환 절차는 `references/pptx-implementation.md § 로고 알파 변환` 참조.
