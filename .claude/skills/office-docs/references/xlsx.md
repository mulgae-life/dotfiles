...
> `openpyxl`, `pandas`, and `markitdown` live in the skill's venv (`~/.local/share/office-docs/venv`) — run every script with that venv's `python`, not the system one. `scripts/setup.sh --check` lists what is missing.
...
`openpyxl`, `pandas`, `markitdown` (in the skill's venv) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`)
