...
| `scripts/office/soffice.py --headless --convert-to pdf deck.pptx` | LibreOffice wrapper — bare `soffice` hangs in some sandboxes and needs a writable profile |

## Creating with pptxgenjs — gotchas

`pptxgenjs` is a global npm package here — check with `scripts/setup.sh --check` and run scripts as `NODE_PATH=$(npm root -g) node script.js`. The model knows the API; these are the footguns:
...
- **Icons:** render `react-icons` to SVG (`ReactDOMServer.renderToStaticMarkup`), rasterize with `sharp` at ≥256px, and insert via `addImage({ data: "image/png;base64," + buf.toString("base64") })` — the `image/png;base64,` prefix is required (`react-icons`, `react`, `react-dom`, and `sharp` are installed globally by `scripts/setup.sh`).
...
```bash
python scripts/unpack.py deck.pptx unpacked/                          # refuses symlink entries and path escapes
...
python scripts/pack.py unpacked/ out.pptx                             # writes a fresh archive, so deleted parts cannot survive
...
mkdir -p render-1                      # a fresh directory per render pass (render-2, render-3, …), so stale images never mix in
pdftoppm -jpeg -r 150 output.pdf render-1/slide
ls -1 "$PWD"/render-1/slide-*.jpg
```

**Pass the absolute paths printed above directly to the view tool.** `pdftoppm` zero-pads based on page count: `slide-1.jpg` for decks under 10 pages, `slide-01.jpg` for 10-99, `slide-001.jpg` for 100+.

**After fixes, rerun all four commands above** — the PDF must be regenerated from the edited `.pptx` before `pdftoppm` can reflect your changes.

## Dependencies

`pptxgenjs` (npm global) · `markitdown[pptx]`, `Pillow`, `defusedxml`, `lxml` (in the skill's venv — text dump, thumbnail, clean, validate) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`) · `pdftoppm` (Poppler) — `scripts/setup.sh --check` lists what is missing.
