...
If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run this script from the skill root (`~/.claude/skills/office-docs/`):
...
- Run this script from the skill root (`~/.claude/skills/office-docs/`): `python scripts/extract_form_field_info.py <input.pdf> <field_info.json>`. It will create a JSON file with a list of fields in this format:
```
[
  {
...
```
- Convert the PDF to PNGs (one image for each page) with this script (run from the skill root, `~/.claude/skills/office-docs/`):
...
```
- Run the `fill_fillable_fields.py` script from the skill root (`~/.claude/skills/office-docs/`) to create a filled-in PDF:
