...
| 카테고리 | 핵심 규칙 요약 |
|----------|---------------|
| **접근성(Accessibility)** | `aria-label`, 시맨틱 HTML, 키보드 핸들러, `aria-live`, skip link, 미디어 자막·대본과 키보드 컨트롤 |
| **포커스 상태(Focus States)** | `focus-visible` 링, `outline-none` 금지, `:focus-within` 활용, sticky 요소가 포커스 요소를 가리지 않기 |
| **폼(Forms)** | `autocomplete`/`name`, 올바른 `type`/`inputmode`, paste 차단 금지, 인라인 에러, 미저장 경고 |
| **애니메이션(Animation)** | `prefers-reduced-motion` 존중, `transform`/`opacity`만 애니메이트, `transition: all` 금지, 5초 넘는 자동재생 모션에 정지 컨트롤 |
...
| **성능(Performance)** | 50+ 항목 가상화, 렌더 중 레이아웃 읽기 금지, `preconnect`/`preload`, GIF 대신 `<video muted loop playsinline>` |
| **내비게이션/상태(Navigation & State)** | URL에 상태 반영, `<a>`/`<Link>` 사용, 파괴적 액션 확인 |
| **터치/인터랙션(Touch & Interaction)** | `touch-action: manipulation`, `overscroll-behavior: contain`, 드래그 중 선택 비활성화, 제스처에 탭·키보드 대안 |
...
| **안티패턴(Anti-patterns)** | zoom 비활성화, paste 차단, `transition: all`, `div` 클릭 핸들러, 압축 영상 대신 GIF, 제스처 전용 액션 등 플래그 |
