...
description: 기능을 보존하면서 코드의 구조와 가독성을 개선합니다. code-review(심층 리포트)와 달리 코드를 직접 수정합니다.
when_to_use: "코드 정리해줘, 리팩토링해줘, 가독성 개선해줘, 코드 단순화해줘 요청 시. 복잡도 감소, 중복 제거, 네이밍/구조 정리가 필요할 때."
---
...
- **기본**: `git status --porcelain`으로 최근 수정된 파일을 식별하여 해당 코드만 개선 (staged·unstaged·untracked 모두 포함 — `git diff --name-only`만으로는 staged/신규 파일이 누락됨)
...
### 1단계: 대상 식별
- `git status --porcelain` (staged·unstaged·untracked 포함) 또는 사용자가 지정한 파일 목록 확인
...
- 각 변경 후 기능 보존 여부 확인 — work-principles의 "리팩토링→전후 동일 테스트 통과"에 따라 단순화 전후 동일 테스트가 통과하는지 확인
- 체크리스트:
  - [ ] 불필요한 중첩 제거
  - [ ] 관련 로직 그룹화
  - [ ] 함수/클래스 단일 책임 원칙
