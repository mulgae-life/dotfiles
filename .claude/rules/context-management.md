...
2. 상세 내용은 `.claude/scratch/`에 저장하고 필요 시 파일에서 재읽기 (Write 자동 승인 — CLAUDE.md 도구 힌트와 동일 방침)

## 계획 영속화

복잡한 작업(3단계 이상)의 계획은 `agent-guide/plans/`(있으면) 또는 `.claude/plans/`에 저장:
- 구현 중 계획 참조 시 파일에서 재읽기 (기억 의존 금지)
- 구현 완료 후 `.archive/`로 이동 (`rm` 금지 — work-principles 산출물 정리 원칙과 동일)
